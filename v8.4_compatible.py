"""Sea of Thieves - Anti-False Positive Horn Detector (Advanced Fusion Engine v8.4).

Moteur de détection hybride quadrilatère combinant la corrélation spectrale (CSV),
l'analyse d'enveloppe de Hilbert, une tolérance de disparité stéréo basse pour 
les cornes centrées, et un filtre de fenêtre sur la variance PSD.
"""
...
# ==============================================================================
#  PANNEAU DE CONFIGURATION DES SEUILS ET MULTIPLICATEURS (CALIBRAGE LOGS V8.4.1)
# ==============================================================================

# --- FILTRE STÉRÉO DIRECTIONNEL ---
MIN_STEREO_DISPARITY: float = 0.02  

# --- CONFIGURATION DE LA SURFACE DE SYNERGIE IMMÉDIATE ("LE BOUM") ---
SYNERGY_PRODUCT_THRESHOLD: float = 0.0400  
SYNERGY_FLOOR_CSV: float = 0.085         
...
# --- CONFIGURATION DU DOSSIER TÉMOIN POUR AUTOHOTKEY ---
FLAG_FILE: str = r"C:\Users\Alexandre\fof_detector\fort_detected.txt"
...
class AdvancedFusionMatcher:
    ...
    def evaluate_hybrid_score(self, live_signal: np.ndarray, stereo_disparity: float) -> Tuple[float, float, float, float]:
        """Analyse le signal et valide via la matrice de surface, la stéréo et le PSD."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="SoT Horn Detector - Advanced Fusion Engine v8.4")
    parser.add_argument("--device", type=int, default=96)
    args = parser.parse_args()

    # MODIFICATION : Nettoyage initial de l'ancien fichier indicateur s'il existe
    try:
        path_flag = Path(FLAG_FILE)
        if path_flag.exists():
            path_flag.unlink()
            print("[*] Ancien fichier indicateur effacé au démarrage.")
    except Exception as e:
        print(f"[!] Impossible de nettoyer le fichier indicateur au démarrage : {e}")
    ...
    print(f"[*] Moteur Hybride v8.4 armé [Tolérance Stéréo Basse Fixée à 2%].")
    print("─" * 135)
    ...
    def detector_loop() -> None:
        ...
        while True:
            ...
            print(
                ...
                f"(CSV: {csv_score:.1f}% , Hilbert: {temp_score:.2f} , Stéréo-Disp: {stereo_disparity:.3f} , PSD-Var: {psd_var:.4f}) | "
                f"Statut: {status_str}"
            )
            ...
            if is_match and not is_silent:
                ...
                if (now - last_alert) > COOLDOWN_SECONDS:
                    ...
                    # MODIFICATION : Création physique du fichier pour forcer l'arrêt immédiat de la macro AHK
                    try:
                        with open(FLAG_FILE, "w", encoding="utf-8") as f:
                            f.write("FORT TROUVÉ")
                        print(f"[*] Fichier indicateur créé avec succès à l'emplacement : {FLAG_FILE}")
                    except Exception as e:
                        print(f"[!] Erreur lors de la création du fichier indicateur : {e}")

...
Configuration optimisée : Lot de 15s, Analyse toutes les 5s, Logs de non-détection.
"""
...
def main() -> None:
    ...
    # MODIFICATION : Buffer réduit à 15 secondes (au lieu de 35s)
    buf_len: int = sr * 15
    ...
    def detector_loop() -> None:
        ...
        # CONSERVATION : Analyse toutes les 5 secondes
        ...
        while True:
            ...
            # Si l'environnement est totalement silencieux
            if raw_rms < NOISE_GATE_RMS:
                print(f"[{current_time}] Aucun son détecté (Ambiance sous le Noise Gate).")
                continue

            # Calcul du spectrogramme de Mel du lot de 15s
            ...
            max_score = float(np.max(corr_2d) / (norm_factor * 0.01))
            ...
            if max_score >= STRICT_2D_CORR_THRESHOLD:
                ...
                # MODIFICATION : Sortie terminal explicite lorsque rien n'est détecté
                print(f"[{current_time}] Analyse effectuée : Rien détecté (Score max: {max_score:.3f} / Seuil: {STRICT_2D_CORR_THRESHOLD})")

"""Sea of Thieves - Anti-False Positive Horn Detector (Advanced Fusion Engine v7.8).

Implémente un moteur de détection hybride à surface de synergie multiplicative.
Le score est forcé à 100% dès que le produit de l'accord harmonique (CSV) et de
l'accord temporel (Hilbert) franchit un seuil de surface critique, idéal pour les
signaux faibles à forte corrélation d'enveloppe.
"""
...
# ==============================================================================
#  PANNEAU DE CONFIGURATION DES SEUILS ET MULTIPLICATEURS (CALIBRAGE ULTIME)
# ==============================================================================

# --- CONFIGURATION DE LA SURFACE DE SYNERGIE IMMÉDIATE ("LE BOUM") ---
# Calcule le produit s_csv * s_temp. 
# Réglé à 0.0620 pour laisser passer ta corne minimale (0.0623) et rejeter le bruit (0.0614)
SYNERGY_PRODUCT_THRESHOLD: float = 0.0620  

# Le double verrou de sécurité : le plancher harmonique élimine directement
# ton pire faux positif (qui est à 8.3%), car il exige au moins 8.6% de CSV.
SYNERGY_FLOOR_CSV: float = 0.086 #0.9 en vrai          
...
class AdvancedFusionMatcher:
    """Moteur d'évaluation multi-critères à déclenchement par surface de synergie."""
    ...
    def evaluate_hybrid_score(self, live_signal: np.ndarray) -> Tuple[float, float, float]:
        """Analyse le signal et valide via la matrice de surface synergique."""
        ...
        # ==============================================================================
        # CRITÈRE DE SYNERGIE CROISÉE ADAPTATIF PAR SURFACE (V7.8)
        # ==============================================================================
        # On calcule le produit des deux axes pour former une courbe de décision souple
        synergy_area = s_csv * s_temp
        
        if synergy_area >= SYNERGY_PRODUCT_THRESHOLD and s_csv >= SYNERGY_FLOOR_CSV:
            # L'accord combiné valide la présence de la corne lointaine
...
def alert(score_combined: float, score_csv: float, score_temp: float, rms: float, label: str) -> None:
    ...
    payload = {
        "content": f"🛡️ **[Fusion Engine V7.8] HORN ADAPTIVE MATCH !**\n"
                   ...
                   f"• **Produit de Surface Calculé** : `{score_csv/100 * score_temp:.4f}` (Seuil: {SYNERGY_PRODUCT_THRESHOLD})\n"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="SoT Horn Detector - Advanced Fusion Engine v7.8")
    ...
    print(f"[*] Moteur de Fusion Matricielle v7.8 en surveillance active...")
    print("─" * 125)

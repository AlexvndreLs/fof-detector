"""Sea of Thieves - Anti-False Positive Horn Detector (Advanced Fusion Engine v8.3).

Moteur de détection hybride tri-critères combinant la corrélation spectrale (CSV),
l'analyse d'enveloppe de Hilbert, la disparité stéréo, et la validation
chirurgicale par la pente de la Densité Spectrale de Puissance (PSD).
"""
...
# ==============================================================================
#  PANNEAU DE CONFIGURATION DES SEUILS ET MULTIPLICATEURS (V8.3 PARAMÉTRAGE)
# ==============================================================================

# --- FILTRE STÉRÉO DIRECTIONNEL ---
MIN_STEREO_DISPARITY: float = 0.02  # Tolère les cornes parfaitement dans l'axe (2%)

# --- FILTRE DE DENSITÉ SPECTRALE (PSD) ---
# Une valeur proche de 0 signifie un spectre parfaitement plat (bruit blanc/rose/chargement).
# La corne, par sa nature harmonique, force une fluctuation de pente importante.
MIN_SPECTRAL_SLOPE_VARIANCE: float = 0.005  # Seuil anti-bruit continu / chargement
...
SYNERGY_FLOOR_CSV: float = 0.085         # Plancher harmonique de sécurité (8.5%)
...
class AdvancedFusionMatcher:
    """Moteur tri-critères à analyse géométrique de spectre et disparité 3D."""
    ...
    def _analyze_spectral_linearity(self, signal: np.ndarray) -> float:
        """Calcule la variance de la pente du PSD pour repérer le bruit uniforme."""
        if signal.size == 0 or np.all(signal == 0):
            return 0.0
        
        frame = signal[:N_FFT] if len(signal) >= N_FFT else np.pad(signal, (0, N_FFT - len(signal)))
        fft_mag = np.abs(np.fft.rfft(frame * self.window, n=N_FFT)) + 1e-9
        psd = fft_mag ** 2
        
        idx_sub = np.where((self.fft_freqs >= 35.0) & (self.fft_freqs <= 500.0))[0]
        if len(idx_sub) < 2:
            return 0.0
            
        log_psd = np.log10(psd[idx_sub])
        norm_freqs = self.fft_freqs[idx_sub] / np.max(self.fft_freqs[idx_sub])
        
        # Régression linéaire pour obtenir la ligne de tendance du bruit de fond
        slope, intercept = np.polyfit(norm_freqs, log_psd, 1)
        expected_log_psd = slope * norm_freqs + intercept
        
        # Mesure l'écart quadratique moyen (les pics de la corne s'écartent fortement de la ligne)
        variance_vs_noise = float(np.mean((log_psd - expected_log_psd) ** 2))
        return variance_vs_noise

    def evaluate_hybrid_score(self, live_signal: np.ndarray, stereo_disparity: float) -> Tuple[float, float, float, float]:
        """Analyse le signal et valide via le produit de surface, la stéréo et le PSD."""
        ...
        if best_window_signal.size > 0:
            ...
            # Calcul de la structure spectrale fine
            psd_variance = self._analyze_spectral_linearity(best_window_signal)

        # ==============================================================================
        # ARBITRAGE DU QUADRUPLE VERROU DE SÉCURITÉ BI-DIMENSIONNEL
        # ==============================================================================
        synergy_area = s_csv * s_temp
        
        if synergy_area >= SYNERGY_PRODUCT_THRESHOLD and s_csv >= SYNERGY_FLOOR_CSV:
            # Sécurité 1 : Si le spectre est ultra-lisse (Variance PSD très basse), c'est du bruit continu
            if psd_variance < MIN_SPECTRAL_SLOPE_VARIANCE:
                return 0.0, max_csv_score, s_temp, psd_variance
                
            # Sécurité 2 : Si la stéréo s'effondre ET que le spectre tend vers un profil lisse, blocage
            if stereo_disparity < MIN_STEREO_DISPARITY and psd_variance < 0.015:
                return 0.0, max_csv_score, s_temp, psd_variance
...
def alert(score_combined: float, score_csv: float, score_temp: float, rms: float, label: str) -> None:
    ...
    payload = {
        "content": f"🛡️ **[Fusion Engine V8.3] ANTI-NOISE ROBUST MATCH !**\n"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="SoT Horn Detector - Advanced Fusion Engine v8.3")
    ...
    print(f"[*] Moteur Hybride v8.3 armé [Filtre de Tendance de Pente PSD Actif].")
    print("─" * 135)
    ...
    def detector_loop() -> None:
        ...
        while True:
            ...
            combined_score, csv_score, temp_score, psd_var = matcher.evaluate_hybrid_score(mono_mix, stereo_disparity)
            ...
            elif combined_score == 0.0 and csv_score > 0.08 and psd_var < MIN_SPECTRAL_SLOPE_VARIANCE:
                status_str = "Bloqué (Bruit d'ambiance continu ou Chargement détecté via PSD)"
            else:
            ...
            print(
                ...
                f"(CSV: {csv_score:.1f}% , Hilbert: {temp_score:.2f} , PSD-Var: {psd_var:.4f}) | "
                f"Statut: {status_str}"
            )

...
T_PYTHON_SHOW          := 15000  ; délai avant affichage fenêtre Python (ms)
T_PYTHON_HIDE          := 15000  ; délai avant masquage fenêtre Python après T_IN_GAME (ms)
...
RunSession() {
    ...
    global T_AFTER_ENTER_TITLE, T_POPUP_ESC, T_PYTHON_SHOW, T_PYTHON_HIDE
    ...
    if !TEST_MODE && SHOW_PYTHON {
        Sleep(T_PYTHON_SHOW)
        WinShow("C:\\miniconda\\envs\\sot\\python.exe")
        WinActivate("C:\\miniconda\\envs\\sot\\python.exe")
    }
    ...
    if !TEST_MODE && SHOW_PYTHON {
        Sleep(T_PYTHON_HIDE)
        WinHide("C:\\miniconda\\envs\\sot\\python.exe")
    }

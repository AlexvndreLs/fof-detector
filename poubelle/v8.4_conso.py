...
# Conservé à 2%. Permet d'attraper les vraies cornes lointaines axées (comme celle à 0.033)
# tout en bloquant les écrans de chargement mono pur (0.000 à 0.010).
MIN_STEREO_DISPARITY: float = 0.02  

# --- CONFIGURATION DE LA SURFACE DE SYNERGIE IMMÉDIATE ("LE BOUM") ---
# Calcule le produit s_csv * s_temp.
# Abaissé à 0.0400 pour capturer l'attaque précoce oubliée à 00:49:51 (qui vaut 0.04095).
SYNERGY_PRODUCT_THRESHOLD: float = 0.0400  

# Plancher harmonique de sécurité conservé à 8.5%.
# Laisse passer tes deux trames critiques à 9.6% et 11.7%, bloque le bruit sous 8.3%.
...
class AdvancedFusionMatcher:
    ...
    def evaluate_hybrid_score(self, live_signal: np.ndarray, stereo_disparity: float) -> Tuple[float, float, float, float]:
        ...
        if synergy_area >= SYNERGY_PRODUCT_THRESHOLD and s_csv >= SYNERGY_FLOOR_CSV:
            ...
            # Bloque le bruit à moyenne variance (0.30 - 0.45) et faible spectre (< 9.2%) [cite: 7]
            if s_csv <= 0.092 and (0.30 <= psd_variance <= 0.45):
                return 0.0, max_csv_score, s_temp, psd_variance
...
def main() -> None:
    ...
    try:
        matcher.load_profiles("horn_profile_local_conso.csv")

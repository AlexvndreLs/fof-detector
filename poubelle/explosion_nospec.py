"""Sea of Thieves - Anti-False Positive Horn Detector (Advanced Fusion Engine v7.6).

Implémente un moteur de détection hybride combinant la corrélation spectrale locale
et l'analyse de profil d'enveloppe par transformée de Hilbert. L'activation des 
scores suit une logique de transition et d'explosion exponentielle en fonction de 
paliers critiques configurables, sans l'utilisation de la pureté spectrale.
"""
...
# Équilibre des forces entre la signature harmonique et la dynamique temporelle.
WEIGHT_HARMONIC_CSV: float = 0.70
WEIGHT_ENVELOPE_HILBERT: float = 0.30
...
# Diviseur/Pénalité de chute si sous le seuil requis (Exposant de compression)
...
# Facteur multiplicateur appliqué en cas de synergie totale croisée (2 axes OK)
CROSS_SYNERGY_BONUS_FACTOR: float = 1.5
...
class AdvancedFusionMatcher:
    """Moteur d'évaluation multi-critères (CSV + Hilbert) à courbe d'activation exponentielle."""
    ...
    def evaluate_hybrid_score(self, live_signal: np.ndarray) -> Tuple[float, float, float]:
        ...
        if s_csv > 0.65 and s_temp > HILBERT_ENV_THRESHOLD:
...
def alert(score_combined: float, score_csv: float, score_temp: float, rms: float, label: str) -> None:
    ...
    payload = {
        "content": f"🛡️ **[Fusion Engine V7.6] HORN ACCELERATION EXPLOSION !**\n"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="SoT Horn Detector - Advanced Fusion Engine v7.6")
    ...
    print(f"[*] Moteur de Fusion (CSV + Hilbert) v7.6 armé et opérationnel.")
    print("─" * 105)

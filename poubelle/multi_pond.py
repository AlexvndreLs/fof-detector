"""
Sea of Thieves - Anti-False Positive Horn Detector (Advanced Fusion Engine v5.2)
Implémente un moteur de détection hybride combinant la corrélation spectrale locale par CSV
et l'analyse de profil d'enveloppe énergétique basse fréquence (Hilbert robuste).
"""
...
# Bornes de filtrage de la corne (Identifiées dans last_try.py)
BANDPASS_LOW: float = 35.0
BANDPASS_HIGH: float = 220.0

# Seuil d'adéquation combiné réajusté pour la fusion (0 à 100 %)
COMBINED_MATCH_THRESHOLD: float = 40.0
...
def get_smooth_envelope(signal: np.ndarray) -> np.ndarray:
    ...
    # Fenêtrage de lissage temporel (Moyenne glissante)
    kernel_size = 256
...
class AdvancedFusionMatcher:
    """Moteur d'évaluation multi-critères : Empreinte CSV et Variance d'enveloppe."""
    ...
    def generate_template_envelope(self, template_wav_path: str) -> None:
        """Génère l'enveloppe de référence temporelle."""
    ...
    def evaluate_hybrid_score(self, live_signal: np.ndarray) -> Tuple[float, float, float]:
        """Analyse le signal et calcule les scores spectraux et d'enveloppe."""
        # 1. Analyse Spectrale Glissante (Basée sur le CSV local)
        ...
        # 2. Analyse Temporelle Robuste (Hilbert + Stabilité de Variance)
        # Évite l'écueil de taille en interpolant ou en bornant la variance du signal filtré
        ...
        if best_window_signal.size > 0:
            ...
            # Mesure alternative : Analyse de l'Énergie Relative Contenue (Sub-band Ratio)
            # Extrait le ratio d'énergie dans la bande utile vs l'énergie totale du segment
            ...
            # Indice de régularité de l'enveloppe (Zéro division)
            env_variance = np.var(live_env)
            ref_variance = np.var(self.template_env) if self.template_env.size > 0 else 0.05
            
            # Score d'adéquation de dynamique temporelle
            temporal_score = float(energy_ratio * (1.0 - min(1.0, abs(env_variance - ref_variance) / (ref_variance + 1e-6))))
            temporal_score = max(0.01, min(1.0, temporal_score))

        # 3. Fusion Multiplicative Pondérée Rééquilibrée
        # Le score temporel est normalisé entre 0 et 1, nous l'appliquons comme atténuateur/amplificateur
        combined_score = max_csv_score * (temporal_score ** 0.5)

        return combined_score, max_csv_score, temporal_score


def alert(score_combined: float, score_csv: float, score_temp: float, rms: float, label: str) -> None:
    print(f"DISCORD ALERTE ENVOYÉE ! | Fusion: {score_combined:.2f}% | Force: {label}")
    if not DISCORD_WEBHOOK:
        return

    payload = {
        "content": f"🛡️ **[Fusion Engine V5.2] HORN VALIDATED !**\n"
                   f"• **Indice de Confiance Combiné** : `{score_combined:.2f}%` (Seuil: {COMBINED_MATCH_THRESHOLD}%)\n"
                   f"• **Précision Harmonique (CSV)** : `{score_csv:.1f}%` \n"
                   f"• **Adéquation Temporelle (Sub-Band)** : `{score_temp:.3f}` \n"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SoT Horn Detector - Advanced Fusion Engine v5.2")
    ...
    try:
        ...
        print(f"[Erreur Critique] Échec du chargement des fichiers de configuration : {e}")
        sys.exit(1)

    print(f"[*] Moteur d'analyse hybride initialisé. Surveillance en cours...")
    print("─" * 100)
    ...
    def detector_loop() -> None:
        ...
        while True:
            ...
            if warmup_cycles > 0:
                print(f"[*] Synchronisation du flux... (Encore {warmup_cycles * 5}s)")
                warmup_cycles -= 1
                continue
            ...
            # Exécution de l'algorithme de fusion de capteurs
            ...
            print(
                ...
                f"COMBINÉ: {combined_score:.2f}%/{COMBINED_MATCH_THRESHOLD:.0f}% | "
                f"(CSV: {csv_score:.1f}% , Temp: {temp_score:.2f}) | "
                f"Statut: {status_str}"
            )
